...
func default() -> GridItemBuilder:
    ...
    self.create_cubes(max_height)
    self.place_scatter(1, 1, int(max_height / 2.0), Graphics.stair_textures["up"])
    self.place_scatter(1, 0, int(max_height / 2.0), Graphics.stair_textures["down"])
    self.place_scatter(200)
    self.place_water(1)
    self.add_cliffs()
...
func place_water(water_level: int) -> GridItemBuilder:
    ...
    for i in grid_items.size():
        if grid_items.heights[i] <= water_level:
            grid_items.update_cube(i, grid_items.positions[i], water_level, Graphics.scatter_textures["water_cube_m"])
...
func place_scatter(num_scatters: int, height_offset: int = 0, min_height: int = 0, texture: Texture2D = null) -> GridItemBuilder:
    ...
    # (3) place scatter 
        grid_items.update_cube(index, grid_items.positions[index], grid_items.heights[index] + height_offset, texture)
